...
from VMAInterpreter.Interpreter import Interpreter
import ErrorLog
class VMACompiler:
    ...
    def compile(self) -> str:
        ...
        VMALines = VMAText.splitlines()
        self.interpreter = Interpreter(VMALines,self.map)
        for prefab_name, prefab_content in self.prefabs.items():
            output = output.replace(f"<{prefab_name}>", f"{prefab_name}\n{prefab_content}")

        
        for i, line in enumerate(VMALines):
            self.interpreter.interpret(line, i + 1)
